 ...
 *   (Anthropic Claude Pro/Max, OpenAI/ChatGPT Codex, Google Antigravity,
 *   Kimi For Coding, Cursor, Qwen/Alibaba, and Ollama) and
 ...
 *
 * Anthropic OAuth (Claude Pro/Max) and Google Antigravity work out of the box:
 * this package owns Antigravity's Google OAuth and Cloud Code Assist stream for
 * the base provider plus each numbered alias (without image generation).
 * It also enables
...
} from "./antigravity/auth/index.js";
import { DEFAULT_ENDPOINT } from "./antigravity/client/index.js";
import { getCurrentAntigravityCatalog } from "./antigravity/models/index.js";
import { ANTIGRAVITY_API, streamAntigravity } from "./antigravity/stream/index.js";
import {
	CURSOR_BASE,
 ...
 */
function unprovisionNativeSlot(provider: string): void {
	try {
...
type AuthEntry = {
	...
	key?: string;
	accountId?: string;
	/** Cloud Code Assist project selected during Antigravity OAuth. */
	projectId?: string;
	/** Stable Google identity used to deduplicate Antigravity login slots. */
	email?: string;
};
...
function accountIdentity(entry: AuthEntry): string | undefined {
	if (
		...
		return `antigravity:${hash12(entry.email.trim().toLowerCase())}`;
	if (entry.access) {
...
/**
 * Refresh one Antigravity slot while preserving its Cloud Code Assist project and
 * recovering from a competing Pi process that refreshed the token first.
 */
async function refreshAntigravityCredentials(
	...
	const refresh = (candidate: AuthEntry) =>
		refreshAntigravityToken(candidate as any) as Promise<AuthEntry>;
	const refreshed = providerId
		? await refreshWithDiskRetry({
				credentials,
				refresh,
...
export function formatAccountIdentifier(raw: string): string {
...
export function maskAccount(raw: string): string {
	return formatAccountIdentifier(raw);
}

export function familyDisplayLabel(family: ProviderFamily): string {
	switch (family) {
		case "openai-codex":
			return "ChatGPT";
		case "antigravity":
...
export function slotAccountDetails(
	id: string,
	...
	let rawPlan: string | undefined = usagePlan;

	if (family === "antigravity") {
		if (typeof entry.email === "string" && entry.email.trim()) {
			rawAccount = entry.email.trim();
		}
		...
		if (typeof entry.access === "string" && entry.access) {
			...
			rawAccount =
				...
				(typeof payload?.sub === "string" && !payload.sub.startsWith("subject:")
					? payload.sub
					: undefined);
			const jwtPlan = payload?.["https://api.openai.com/auth"]?.chatgpt_plan_type;
			if (typeof jwtPlan === "string" && jwtPlan.trim()) {
				rawPlan = jwtPlan.trim().toLowerCase();
			}
		}
		if (!rawAccount && typeof entry.accountId === "string") {
			rawAccount = entry.accountId;
		}
	} else if (family === "cursor") {
		if (typeof entry.access === "string" && entry.access) {
			const payload = decodeJwtPayload(entry.access);
			rawAccount = payload?.email || payload?.sub;
		}
	} else {
		if (typeof entry.email === "string" && entry.email.trim()) {
			rawAccount = entry.email.trim();
		} else if (typeof entry.accountId === "string") {
			rawAccount = entry.accountId;
		}
	}

	if (rawPlan && rawPlan.toLowerCase() === family.toLowerCase()) {
		rawPlan = undefined;
	}

	return { account: rawAccount, plan: rawPlan };
}

export function formatSlotDisplayName(
	id: string,
	entry?: AuthEntry,
	family?: ProviderFamily,
	usagePlan?: string,
	...
	const resolvedFamily =
		family ?? classifyProvider(id, DEFAULT_QWEN_PROVIDER) ?? "openai-codex";
	const index = slotIndex(id);
	const label = familyDisplayLabel(resolvedFamily);
	const baseName = `${label}-${index}`;

	const { account, plan } = slotAccountDetails(
		id,
		entry,
		resolvedFamily,
		usagePlan,
	...
	const tags: string[] = [];
	if (account) tags.push(formatAccountIdentifier(account));
...
function registerAnthropicSlot(
	...
	if (id === ANTHROPIC_BASE && !displayName) return; // base provider: oauth + shaping registered in piMultiAccount()
	const name =
		displayName ??
		formatSlotDisplayName(id, readAuthFile()[id], "anthropic");
...
function registerCodexSlot(
	...
	if (id === CODEX_BASE && !displayName) return; // base provider is native until live catalog sync enriches it
	const name =
		displayName ??
		formatSlotDisplayName(id, readAuthFile()[id], "openai-codex");
	pi.registerProvider(id, {
		name,
...
/**
 * Antigravity uses its own Cloud Code Assist transport rather than Pi's native
 * OpenAI/Anthropic transports. Every slot has the same provider definition but
 * its own OAuth credential and project id, which `getApiKey` serializes for the
 * stream at request time.
 *
 * Dynamic model discovery remains intentionally disabled here: the upstream
 * catalog module stores one global routing table, whereas a Google plan can give
 * different Antigravity accounts different catalogs. Static models plus the
 * stream's authenticated runtime lookup avoid leaking one account's entitlement
 * into another slot.
 */
function antigravityOAuthOverride(providerId: string, name: string) {
	return {
		name,
		async login(callbacks: any) {
			return rejectDuplicateLogin(providerId, await loginAntigravity(callbacks));
		},
		refreshToken: (credentials: AuthEntry) =>
			refreshAntigravityCredentials(credentials, providerId),
		getApiKey: (credentials: AuthEntry) => getAntigravityApiKey(credentials as any),
	};
}

function antigravitySlotModels(): Array<Record<string, unknown>> {
	return getCurrentAntigravityCatalog().models.map((model) => ({ ...model }));
}

function registerAntigravitySlot(
	pi: ExtensionAPI,
	id: string,
	displayName?: string,
) {
	const name =
		displayName ??
		formatSlotDisplayName(id, readAuthFile()[id], "antigravity");
	pi.registerProvider(id, {
		name,
		baseUrl: DEFAULT_ENDPOINT,
		api: ANTIGRAVITY_API,
		oauth: antigravityOAuthOverride(id, name),
		models: antigravitySlotModels() as any,
...
function registerKimiSlot(
	...
	if (id === KIMI_BASE && !displayName) return; // base provider is native to Pi (pi-ai ships it)
	const name =
		displayName ??
		formatSlotDisplayName(id, readAuthFile()[id], "kimi-coding");
	pi.registerProvider(id, {
		name,
		baseUrl: KIMI_BASE_URL,
		api: "anthropic-messages" as any,
		oauth: kimiOAuthOverride(id, name),
...
function registerApiKeySlot(
	...
	if (id === baseId && !displayName) return;
	...
	const name = displayName ?? formatSlotDisplayName(id, entry, family);
	pi.registerProvider(id, {
		name,
		baseUrl,
...
export default function piMultiAccount(pi: ExtensionAPI) {
	...
	async function forceRefreshProvider(
		...
		try {
			...
			if (family === "anthropic") {
				...
				refreshed = mergeRefreshedCredentials(
					entry,
					await refreshAntigravityCredentials(entry, provider),
				);
			} else {
				return { status: "unsupported" };
			}
	...
	async function syncCodexModelCatalog(ctx: any, force = false) {
		...
		const duplicateIds = new Set(
			discoverDuplicateSlots(auth).map(({ duplicate }) => duplicate),
		);
	...
	function syncRegisteredSlots(auth: Record<string, AuthEntry>, ctx?: any) {
		// Pi's login menu labels every registered provider that has an auth.json entry
		// as "stored". Rotation already excludes duplicate real accounts, but exposing a
		// redundant OAuth alias here makes the picker look like the user has an extra
		// account and invites a second login into it. Keep the credential on disk (never
		// delete user auth implicitly) while withholding only the duplicate provider slot.
		const duplicateIds = new Set(
			discoverDuplicateSlots(auth).map(({ duplicate }) => duplicate),
		);
		for (const id of duplicateIds) {
			const family = classifyProvider(id, config.qwenProvider);
			// Base providers and Cursor are owned by Pi/its optional companion extension;
			// only unregister aliases that this extension previously registered itself.
			if (!family || family === "cursor" || slotIndex(id) <= 1)
				continue;
			registeredSlots.delete(id);
			pi.unregisterProvider(id);
			unprovisionNativeSlot(id);
		}
		...
		for (const family of families) {
			...
			if (
				...
				let spare = 2;
				// A redundant credential remains in auth.json for explicit user cleanup. Do
				// not reuse its number for the spare slot, or Pi would immediately render the
				// hidden duplicate as another "stored" login target.
				while (
					authedIndexes.includes(spare) &&
					spare <= config.maxAccountsPerProvider
				)
					spare++;
			...
			for (const index of wanted) {
				...
				const entry = auth[id];
				const usagePlan =
					cachedUsage(id)?.plan ?? usageByProvider.get(id)?.plan;
				const displayName = formatSlotDisplayName(
					id,
					entry,
					family,
					usagePlan,
				...
				if (index === 1) {
					if (family === "anthropic") {
						pi.registerProvider("anthropic", {
							name: displayName,
							oauth: anthropicOAuthOverride("anthropic", displayName),
						} as any);
					} else if (family === "openai-codex") {
						pi.registerProvider("openai-codex", {
							name: displayName,
							oauth: codexOAuthOverride("openai-codex", displayName),
						} as any);
					} else if (family === "antigravity") {
						registerAntigravitySlot(pi, "antigravity", displayName);
					}
					continue;
				}
				...
				if (family === "anthropic")
					registerAnthropicSlot(pi, id, undefined, displayName);
				else if (family === "openai-codex") {
					...
					registerAntigravitySlot(pi, id, displayName);
				} else if (family === "kimi-coding") {
					const kimiModels = [
						...new Set([...DEFAULT_KIMI_MODELS, ...hostModelIdsFor(ctx, KIMI_BASE)]),
					];
					registerKimiSlot(pi, id, kimiModels, displayName);
	...
	function compactAuthSlots(targetFamily?: ProviderFamily): Array<{ from: string; to: string }> {
		const auth = readAuthFile();
		const families: ProviderFamily[] = targetFamily
			? [targetFamily]
			: [
					"anthropic",
					"openai-codex",
		...
		const allShifted: Array<{ from: string; to: string }> = [];
		...
		for (const family of families) {
			...
			for (const id of slots) {
				const expectedId = slotId(family, nextIndex, config.qwenProvider);
				if (id !== expectedId) {
					auth[expectedId] = auth[id];
					delete auth[id];
					allShifted.push({ from: id, to: expectedId });
		...
		if (changed) {
			try {
				writeFileSync(AUTH_PATH, `${JSON.stringify(auth, null, "\t")}\n`, {
					encoding: "utf8",
					mode: 0o600,
				});
			} catch {
			...
			if (existsSync(MODELS_CONFIG_PATH)) {
				try {
					const modelsRaw = readFileSync(MODELS_CONFIG_PATH, "utf8");
					const models = JSON.parse(modelsRaw);
					if (models.providers) {
						let modelsChanged = false;
						for (const { from, to } of allShifted) {
							if (models.providers[from]) {
								const entry = { ...models.providers[from] };
								if (typeof entry.baseUrl === "string") {
									entry.baseUrl = entry.baseUrl.replaceAll(from, to);
								}
								models.providers[to] = entry;
								delete models.providers[from];
								modelsChanged = true;
							}
						}
						if (modelsChanged) {
	...
	async function handleCommand(args: string, ctx: any) {
		...
		if (command === "compact" || command === "defrag") {
			const family = parseFamilyArg(arg1);
			const shifted = compactAuthSlots(family);
			if (shifted.length > 0) {
				reloadHostAuth(ctx);
				refreshDiscovery(true, ctx);
				const details = shifted
					.map(({ from, to }) => `${from} → ${to}`)
					.join(", ");
				ctx.ui.notify(
					`pi-multi-account: compacted slots (${details}). Rotation: ${rotation.join(" → ") || "none"}`,
					"info",
				);
			} else {
				ctx.ui.notify(
					"pi-multi-account: all slots are already contiguous (no gaps to compact).",
					"info",
				);
			}
			return;
		...
		if (command === "add") {
			...
			if (!family) {
				ctx.ui.notify(
					"pi-multi-account: usage: /multi-account add <anthropic|codex|kimi|antigravity|cursor|ollama|qwen>",
					"warning",
				);
				return;
			}
			...
			if (
				...
				const slotLabel = formatSlotDisplayName(id);
				const loginHint =
					family === "cursor"
						? `run /login, authenticate your Cursor subscription in the browser, select ${slotLabel} (${id})`
						: family === "antigravity"
							? `run /login, select ${slotLabel} (${id}), then complete Google sign-in in the browser`
							: `run /login, choose "Use a subscription", select ${slotLabel} (${id})`;
				ctx.ui.notify(
		...
		if (command === "remove" || command === "rm" || command === "delete") {
			...
			if (!targetRaw) {
				ctx.ui.notify(
					"pi-multi-account: usage: /multi-account remove <anthropic|codex|kimi|antigravity|cursor|ollama|qwen|provider-id>",
					"warning",
				);
				return;
			}
		...
		ctx.ui.notify(
			[
				...
				`Other commands: status | best | priority [...] | limits [refresh] | models | log [N|on|off] | only-active [on|off] | rediscover | add [anthropic|codex|kimi|antigravity|cursor|ollama|qwen] | remove [anthropic|codex|kimi|antigravity|cursor|ollama|qwen|<provider-id>] | revive <provider|all> | clear | stop | reset | reload | enable | disable`,
			].join("\n"),
			"info",
		);
	}
	...
	for (const name of ["limits", "quota"]) {
		pi.registerCommand(name, {
			description: "Show multi-account limits & quota",
			handler: (args: string, ctx: any) =>
				safeHandleCommand(`limits${args.trim() ? ` ${args.trim()}` : ""}`, ctx),
		});
	}

	// ----- Antigravity multi-account provider -------------------------------
	// The provider-local streamSimple handler owns the custom Cloud Code Assist
	// transport for the base provider and every numbered alias. No image API,
	// image command, or image-generation tool is registered here.
	const initialAuth = readAuthFile();
	registerAntigravitySlot(
		pi,
		"antigravity",
		formatSlotDisplayName("antigravity", initialAuth["antigravity"], "antigravity"),
	);
	...
	const initialAnthropicName = formatSlotDisplayName(
		"anthropic",
		initialAuth["anthropic"],
		"anthropic",
	);
	pi.registerProvider("anthropic", {
		name: initialAnthropicName,
		oauth: anthropicOAuthOverride("anthropic", initialAnthropicName),
	} as any);

	const initialCodexName = formatSlotDisplayName(
		"openai-codex",
		initialAuth["openai-codex"],
		"openai-codex",
	);
	pi.registerProvider("openai-codex", {
		name: initialCodexName,
		oauth: codexOAuthOverride("openai-codex", initialCodexName),
	} as any);
	...
	const ensureApiKeyBaseProvider = (
		...
		const displayName = formatSlotDisplayName(baseId, entry, family);
